...
def create_query(user_query, click_prior_query, filters, synonyms=False, sort="_score", sortDir="desc", size=10, source=None):
    query_obj = {
        'size': size,
        "sort": [
            {sort: {"order": sortDir}}
    ...
    # Replace name match query with name.synonyms match query
    if synonyms:
...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc", synonyms=False):
    ...
    query_obj = create_query(user_query, synonyms=synonyms, click_prior_query=None, filters=None, sort=sort, sortDir=sortDir, source=["name", "shortDescription"])
...
if __name__ == "__main__":
    ...
    general.add_argument('--synonyms', action='store_true', help='Query with synonyms')
